...
$assigned_query = "SELECT COUNT(*) as total FROM tickets WHERE assigned_division_id = $division_id AND status = 'Assigned'";
$assigned_result = mysqli_query($conn, $assigned_query);
$assigned_count = mysqli_fetch_assoc($assigned_result)['total'];

$stmt = mysqli_prepare($conn, "SELECT u.*, d.nama_divisi FROM users u LEFT JOIN divisions d ON u.division_id = d.id WHERE u.id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (!$errors) {
        ...
        if (mysqli_num_rows($check) > 0) { $errors[] = 'Username sudah digunakan.'; }
        mysqli_stmt_close($stmt);
    }
    if (!$errors) {
...
$total_div_tickets = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tickets WHERE assigned_division_id = $division_id"))['c'];
$progress_tickets = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tickets WHERE assigned_division_id = $division_id AND status='In Progress'"))['c'];
$resolved_tickets = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tickets WHERE assigned_division_id = $division_id AND status='Resolved'"))['c'];
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Profil Teknisi - Ticketing TVRI</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .role-badges{display:flex;flex-wrap:wrap;gap:6px;justify-content:center;margin-bottom:18px}
        .role-badge{display:flex;align-items:center;gap:5px;background:#eff6ff;color:#10367D;border:1px solid #dbeafe;padding:4px 14px;border-radius:20px;font-size:.76rem;font-weight:600}
        .div-badge{display:flex;align-items:center;gap:5px;background:#fef3c7;color:#92400e;border:1px solid #fde68a;padding:4px 14px;border-radius:20px;font-size:.76rem;font-weight:600}
...
<body>
    ...
    <nav class="sidebar">
        ...
        <div class="sidebar-nav">
            <a class="nav-link position-relative" href="dashboard.php"><i class="bi bi-grid-1x2"></i> Dashboard<?php if($assigned_count > 0): ?><span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-primary" style="font-size:0.7em"><?=$assigned_count?></span><?php endif; ?></a>
        ...
        <div class="sidebar-footer">
            ...
            <div class="division-badge"><i class="bi bi-tools"></i> <?= htmlspecialchars($user['nama_divisi']) ?></div>
        ...
        <div class="profile-layout">
            <div class="profile-card">
                ...
                <div class="profile-card-body">
                    ...
                    <div class="role-badges">
                        <div class="role-badge"><i class="bi bi-tools"></i> Teknisi</div>
                        <?php if(!empty($userData['nama_divisi'])): ?><div class="div-badge"><i class="bi bi-building"></i> <?= htmlspecialchars($userData['nama_divisi']) ?></div><?php endif; ?>
                    </div>
                    ...
                    <div class="view-section" id="viewPersonal">
                        ...
                        <div class="info-grid">
                            ...
                            <div class="info-item"><div class="info-label">Divisi</div><div class="info-value"><?= htmlspecialchars($userData['nama_divisi'] ?? '-') ?></div></div>
            ...
            <div class="right-col">
                <div class="card-panel">
                    <div class="card-panel-title"><i class="bi bi-bar-chart-line"></i> Statistik Divisi</div>
                    <div class="stats-grid">
                        <div class="stat-box"><div class="stat-icon" style="background:#eff6ff;color:#10367D"><i class="bi bi-ticket-perforated-fill"></i></div><div class="stat-num" style="color:#10367D"><?= $total_div_tickets ?></div><div class="stat-lbl">Total Tiket</div></div>
                        <div class="stat-box"><div class="stat-icon" style="background:#dbeafe;color:#3b82f6"><i class="bi bi-person-check-fill"></i></div><div class="stat-num" style="color:#3b82f6"><?= $assigned_count ?></div><div class="stat-lbl">Assigned</div></div>
                        <div class="stat-box"><div class="stat-icon" style="background:#fef3c7;color:#f59e0b"><i class="bi bi-arrow-repeat"></i></div><div class="stat-num" style="color:#f59e0b"><?= $progress_tickets ?></div><div class="stat-lbl">In Progress</div></div>
