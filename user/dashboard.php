...
$active_query = "SELECT COUNT(*) as total FROM tickets WHERE user_id = ? AND user_hidden = 0 AND (status = 'Assigned' OR status = 'In Progress')";
...
$user_id = $user['id'];
$query_total = "SELECT COUNT(*) as total FROM tickets WHERE user_id = $user_id AND user_hidden = 0";
$query_open = "SELECT COUNT(*) as total FROM tickets WHERE user_id = $user_id AND user_hidden = 0 AND status = 'Open'";
$query_progress = "SELECT COUNT(*) as total FROM tickets WHERE user_id = $user_id AND user_hidden = 0 AND status IN ('Assigned', 'In Progress')";
$query_resolved = "SELECT COUNT(*) as total FROM tickets WHERE user_id = $user_id AND user_hidden = 0 AND status = 'Resolved'";

$total_tickets = mysqli_fetch_assoc(mysqli_query($conn, $query_total))['total'];
$open_tickets = mysqli_fetch_assoc(mysqli_query($conn, $query_open))['total'];
...
$query_tickets = "SELECT t.*, d.nama_divisi, u.foto as pelapor_foto, TIMESTAMPDIFF(SECOND, t.updated_at, NOW()) as seconds_ago FROM tickets t LEFT JOIN divisions d ON t.assigned_division_id = d.id LEFT JOIN users u ON t.user_id = u.id WHERE t.user_id = $user_id AND t.user_hidden = 0 ORDER BY t.created_at DESC LIMIT $limit OFFSET $offset";
...
<head>
    ...
    <title>Dashboard User - Ticketing TVRI</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sidebar{background:linear-gradient(to bottom,#2563eb 0%,#1a4fa0 50%,#10367D 100%);min-height:100vh;color:#fff;position:fixed;top:0;left:0;width:230px;z-index:100;display:flex;flex-direction:column}
        ...
        .btn-new-ticket{background:#10367D;color:#fff;padding:9px 18px;border-radius:10px;font-size:.84rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:6px;transition:all .2s;border:none;cursor:pointer;white-space:nowrap}
        .btn-new-ticket:hover{background:#0c2d68;color:#fff;transform:translateY(-1px);box-shadow:0 4px 12px rgba(16,54,125,.25)}
        ...
        .ticket-table{width:100%;border-collapse:collapse;min-width:920px}
        ...
        .btn-print{background:#eff6ff;color:#2563eb;border:1px solid #dbeafe}.btn-print:hover{background:#dbeafe;color:#1d4ed8}.btn-delete{background:#fef2f2;color:#dc2626;border:1px solid #fecaca}.btn-delete:hover{background:#fee2e2;color:#b91c1c}
        ...
        .empty-state a{color:#10367D;font-weight:600}
        ...
        .hero-subtitle{font-size:.88rem;color:rgba(255,255,255,.75);margin:0 0 22px;max-width:420px;line-height:1.6}
        .btn-hero{background:#fff;color:#10367D;padding:11px 24px;border-radius:12px;font-size:.85rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:all .2s;border:none;cursor:pointer}
        .btn-hero:hover{background:rgba(255,255,255,.92);color:#0c2d68;transform:translateY(-2px);box-shadow:0 4px 15px rgba(0,0,0,.2)}
        ...
        @media(max-width:768px){
            ...
            .btn-hero{width:100%;justify-content:center;padding:12px 18px}
            ...
            .btn-new-ticket{width:100%;justify-content:center;padding:12px 18px;font-size:.88rem;border-radius:12px}
...
<body>
    ...
    <nav class="sidebar">
        ...
        <div class="sidebar-nav">
            <a class="nav-link active position-relative" href="dashboard.php"><i class="bi bi-grid-1x2"></i> Dashboard<?php if($active_count > 0): ?><span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-warning" style="font-size:0.7em"><?=$active_count?></span><?php endif; ?></a>
    ...
    <div class="main-content">
        <div class="hero-banner">
            <div class="hero-content">
                ...
                <p class="hero-subtitle">Pantau dan kelola status tiket Anda dengan mudah melalui dashboard ini</p>
                <a href="lapor.php" class="btn-hero"><i class="bi bi-plus-lg"></i> Buat Tiket Baru</a>
            </div>
        ...
        <div class="stats-row">
            ...
            <div class="stat-card sc-red">
                ...
                <i class="bi bi-exclamation-circle-fill sc-bg"></i>
            </div>
        ...
        <div class="charts-row">
            <div class="chart-card">
                <h3><i class="bi bi-pie-chart-fill"></i> Status Tiket Saya</h3>
        ...
        <div class="table-card">
            <div class="table-card-header">
                <h2><i class="bi bi-list-ul" style="margin-right:6px"></i>My Ticket</h2>
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                    ...
                    <input type="text" id="searchTicket" placeholder="Cari tiket..." style="padding:8px 14px;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;width:280px;font-family:'Inter',sans-serif" onkeyup="filterTickets()">
                    <span class="ticket-count"><?= $total_tickets ?> tiket &middot; Hal <?= $page ?>/<?= max(1,$total_pages) ?></span>
                </div>
            </div>
            <?php if(mysqli_num_rows($result_tickets) > 0): ?>
            <div class="table-wrap">
            <table class="ticket-table">
                <thead><tr><th class="cb-cell hidden"><input type="checkbox" id="selectAll" onchange="toggleSelectAll(this)" style="cursor:pointer"></th><th>Title</th><th>Status</th><th>Priority</th><th>Divisi</th><th>Updated</th><th>Created</th><th>Action</th></tr></thead>
                <tbody>
                ...
                <tr class="ticket-row" tabindex="0" onclick="handleRowClick(event,this,'<?= $url ?>')" onkeydown="handleRowKeydown(event,this,'<?= $url ?>')" role="link" aria-label="Tiket #<?= $t['id'] ?>">
                    ...
                    <td><span class="reporter-cell"><i class="bi bi-diagram-3"></i> <?= htmlspecialchars($t['nama_divisi'] ?? '-') ?></span></td>
                    ...
                    <td>
                        ...
                        <a href="../cetak.php?id=<?= $t['id'] ?>" class="btn-action btn-print" onclick="event.stopPropagation()" target="_blank"><i class="bi bi-printer"></i> Cetak PDF</a>
                        <?php if($t['status'] == 'Resolved'): ?>
                        <button type="button" class="btn-action btn-delete" onclick="event.stopPropagation();deleteTicket(<?= $t['id'] ?>,'<?= htmlspecialchars(addslashes($t['judul'])) ?>')" style="background:#fee;color:#dc2626;border:1px solid #fecaca"><i class="bi bi-trash"></i> Hapus</button>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endwhile; ?>
            ...
            <?php endif; ?>
            <?php else: ?>
            <div class="empty-state"><i class="bi bi-inbox"></i><p>Belum ada tiket. <a href="lapor.php">Buat tiket pertama</a></p></div>
            <?php endif; ?>
        </div>
        </form>
    </div>
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script><script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js"></script>
    <script>
    ...
    let prev={total:<?= $total_tickets ?>,open:<?= $open_tickets ?>,progress:<?= $progress_tickets ?>,resolved:<?= $resolved_tickets ?>};
    function poll(){fetch('../api/check_tickets.php?since='+encodeURIComponent(lastPoll)).then(r=>r.json()).then(d=>{lastPoll=d.timestamp;const c=d.counts;if(c.total!==undefined){anim('statTotal',prev.total,c.total);anim('statOpen',prev.open,c.open);anim('statProgress',prev.progress,c.progress);anim('statResolved',prev.resolved,c.resolved);}if(d.new_items&&d.new_items.length>0&&(c.total!==prev.total||c.resolved!==prev.resolved)){d.new_items.forEach(it=>{toast('bi-bell-fill text-primary','Tiket #'+it.id,it.judul+'  '+it.status);});}prev={total:c.total||0,open:c.open||0,progress:c.progress||0,resolved:c.resolved||0};}).catch(()=>{});}
    ...
    function deleteTicket(ticketId, ticketTitle) {
        Swal.fire({
            title: 'Hapus Tiket?',
            html: 'Anda akan menghapus tiket:<br><strong>"' + ticketTitle + '"</strong><br><br>Tindakan ini tidak dapat dibatalkan!',
            icon: 'warning',
    ...
    if (urlParams.get('success') === 'deleted') {
        Swal.fire({icon:'success',title:'Berhasil',text:'Tiket berhasil dihapus.',timer:3000,showConfirmButton:false});
    } else if (urlParams.get('success') === 'bulk_delete') {
        ...
        Swal.fire({icon:'error',title:'Gagal',text:'Gagal menghapus tiket. Silakan coba lagi.',timer:3000,showConfirmButton:false});
    } else if (urlParams.get('error') === 'not_found') {
        Swal.fire({icon:'error',title:'Error',text:'Tiket tidak ditemukan atau tidak dapat dihapus.',timer:3000,showConfirmButton:false});
